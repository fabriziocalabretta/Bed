...
public class App extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		messages = ResourceBundle.getBundle("org.fc.bed.messages");
...
//		scene.getStylesheets().add(stylesheet);
	...
	OpenFileCommand handleCommandLine(String[] argv) {
		if (argv.length > 0) {
			...
			if (file == null) {
				App.wrongParms("missing file name");
			}
	...
	public void doAbout() {
		...
		alert.setHeaderText(org.fc.bed.PackageInfo.getName());
		alert.setContentText("Version " + org.fc.bed.PackageInfo.getVersion());
		alert.showAndWait();
	}
